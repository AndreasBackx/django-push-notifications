...
from push_notifications.wns import (dict_to_xml_schema, wns_send_bulk_message,
                                    wns_send_message)
...
class WNSSendMessageTestCase(TestCase):
    ...
    def test_send_message_calls_wns_send_with_application_id(self, mock_method, _):
        wns_send_message(uri="one", message="test message", client_id=None,
                         client_secret=None, application_id="123456")
        mock_method.assert_called_with(
            client_id=None,
            client_secret=None,
